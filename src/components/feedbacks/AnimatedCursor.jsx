...
function AnimatedCursorCircle({ targetX, targetY, hoveringDiv, isClicked }) {
  ...
  const _setVisible = (visible) => {
    ...
    if (visible) circleDiv.classList.remove(`d-none`);
    else circleDiv.classList.add(`d-none`);
  };
  ...
  const _setHighlighted = (highlighted) => {
    ...
    else circleDiv.classList.remove(`animated-cursor-circle-highlight`);
  };

  const _update = () => {
    const circleDiv = document.getElementById("animated-cursor-circle");
    if (!circleDiv || !dirty) return;

    const dt = utils.isSafari() ? 1.25 : 1;

    const isPositioned = targetX === curX && targetY === curY;
    if (isPositioned) setStoppedFor((prevState) => prevState + 1 / 60);
    ...
    circleDiv.style.transform = `translate3d(${
      curX - CIRCLE_SIZE_IN_PIXELS / 2
    }px, ${curY - CIRCLE_SIZE_IN_PIXELS / 2}px, 0) scale(${curScale / 3})`;
    circleDiv.style.opacity = curOpacity.toString();
  };

  const _getTargetScale = () => {
    let targetScale = MIN_SCALE;
    if (hoveringDiv) targetScale = MIN_SCALE + (MAX_SCALE - MIN_SCALE) / 2;
    if (isClicked) targetScale = MAX_SCALE;
    return targetScale;
  };
  ...
  const _getTargetOpacity = () => {
    ...
    let targetOpacity = 0;
    if (!shouldHideCompletely) targetOpacity = MIN_OPACITY;
    if (hoveringDiv) targetOpacity = (MAX_OPACITY - MIN_OPACITY) / 2;
    if (isClicked) targetOpacity += (MAX_OPACITY - MIN_OPACITY) / 2;
  ...
  const _detectIcon = () => {
    if (!hoveringDiv) {
      if (!isClicked) setCurFaIcon(null);
      return;
    }
  ...
  return (
    <div className={`animated-cursor-circle`} id={`animated-cursor-circle`}>
      <FaIcon
        iconName={curFaIcon || "fa-solid fa-circle"}
...
function AnimatedCursorTooltip({ hoveringDiv }) {
  ...
  const _detectLabel = () => {
    if (!hoveringDiv) return;

    if (
      hoveringDiv.matches("a") &&
      hoveringDiv.href &&
      hoveringDiv.href.includes("//") &&
      utils.isUrlExternal(hoveringDiv.href)
    ) {
      //setCurLabel("🔗")
      setCurLabel("");
      return;
  ...
  const _update = () => {
    const circleDiv = document.getElementById("animated-cursor-circle");
    const tooltipDiv = document.getElementById("animated-cursor-tooltip");
    if (!circleDiv || !tooltipDiv || !dirty) return;
    ...
    const newX =
      circleBounds.x + circleBounds.width / 2 - tooltipBounds.width / 2;
    const newY = circleBounds.y - 35;
    tooltipDiv.style.transform = `translate3d(${newX}px, ${newY}px, 0)`;
  };

  _update();
  return <Tooltip id={`animated-cursor-tooltip`} label={curLabel} />;
}
